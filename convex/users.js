...
function permissionToRole(permission) {
    if (permission === "manager" || permission === "mentor")
        return "admin";
    if (permission === "team_lead" || permission === "subteam_lead")
        return "pit";
    return "viewer";
}
/**
...
export const upsertOwnRole = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_user", (q) => q.eq("userId", userId))
            .first();
        const RANK = { admin: 2, pit: 1, viewer: 0 };
        // Don't downgrade a role that is already higher.
        if (existing && RANK[existing.role] >= RANK[args.role])
            return;
        if (existing) {
            await ctx.db.patch(existing._id, { role: args.role });
        }
