...
async function requireWriteRole(ctx: { db: any }) {
  ...
  return { userId, role: role.role as "admin" | "pit" };
}
...
function healthScore(restingVoltage?: number, internalResistance?: number) {
  if (restingVoltage == null || internalResistance == null) return undefined;
  let v = 0;
  if (restingVoltage >= 12.9) v = 50;
  else if (restingVoltage >= 12.7) v = 40;
  else if (restingVoltage >= 12.5) v = 25;
  else v = 10;
  let r = 0;
  if (internalResistance < 13) r = 50;
  else if (internalResistance < 18) r = 30;
  else if (internalResistance < 22) r = 15;
  else r = 5;
